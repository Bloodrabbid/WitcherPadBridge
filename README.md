...
На маленьком экране — Steam Deck, ROG Ally — интерфейс мелкий. Масштаб всего интерфейса игра
не умеет: он растягивается на весь экран, поэтому занимает одну и ту же долю при любом
разрешении, а увеличивать панели поштучно значит ломать их вёрстку. Зато размер ТЕКСТА
поднимается штатно, таблицей `fonts.2da`.

```
tools/bigger_text.sh on      # включить
tools/bigger_text.sh off     # вернуть как было
```

Кладёт две таблицы в `Data/2DA/` и ничего больше не трогает; выключение удаляет их.
Читаются они только при старте, так что игру надо перезапустить.

Потолок здесь не наш. Игра не рисует шрифт нужного размера, а берёт готовый атлас из
`System/__cache/`; своих шрифтов у неё нет и новые размеры она не печёт. Поэтому взять можно
только то, что уже испечено. **Для русского список ещё короче**: годятся лишь размеры, которые
использует штатная русская таблица, — в остальных атласах нет кириллицы, и текст тогда
размечается, но не рисуется. Отсюда прибавка скромная: основной текст панелей и реплики в
диалоге примерно на четверть, заголовки на четверть.

Длинные названия в списках после этого обрезаются — ширина списка фиксированная. Полное
название видно в подсказке, которая всплывает на элементе под фокусом.

Русская таблица проверена на живой игре. Латинская собрана тем же способом, но проверить её
было не на чем — если текст в английской сборке пропадёт, выключите обратно и напишите.
