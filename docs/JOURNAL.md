...
## Что проверено
Обе сборки чистые с `-Wall -Wextra`, все шесть скриптов компилируются, пакет собирается.
Lua-половина проверена вживую горячей загрузкой: `rum: vibration layer loaded`,
`rum: hooks installed (camera shake, hit)`, `rum: 0.80/0.20 200ms`, и в
`System/wxp_rumble.txt` появилась строка `1 800 200 200`.
