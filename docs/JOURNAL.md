...
## Проверено на живом железе: опубликованный артефакт, а не локальная сборка
Скачал zip прямо из релиза, сверил sha256, распаковал на Ally и поставил. Установщик правильно
опознал уже выставленный `winxp` и не тронул реестр, `gamepad.ini` с порогом 0.85 не перезаписал,
семь `.luc` разложил. Игра при этом БЫЛА ЗАПУЩЕНА и пережила это (мост продолжал писать `alive:`,
Lua тикать) — но так делать не надо: `cp` пишет в тот же inode, а Wine держит DLL отображённым.
Это линуксовая форма того же правила, что и «не переподписывать .app при запущенной игре».

🔴 CI публикует релиз сам по тегу и пишет ШАБЛОННЫЕ notes (одинаковые для всех версий). После
`git push origin <tag>` тело релиза надо заменить (`gh release edit --notes-file`), иначе
обязательное предупреждение про версию Windows до пользователя не доедет.

---
# 🔴 ИНВЕНТАРЬ БЫЛ НЕДОСТУПЕН С ПАДА НА ЖИВОМ СЕЙВЕ (0.6 → 0.6.1)

Нашлось случайно, при съёмке скриншотов для Steam-гайда. Открыл инвентарь на реальном сейве —
в кольце `equipment[13] / filters[10] / container[1]`. Ни сумки, ни квестовых. При этом
`lm_tRepository` содержал **71 занятую ячейку**.

То есть в опубликованном 0.6 игрок открывает самую нужную панель в игре и не может добраться
ни до одного предмета. На прологовом сейве, где всё и тестировалось, сумка была почти пуста, а
25 августа проверка делалась уже по давно открытой панели — поэтому баг и не всплыл.

## Причина: кольцо — это снимок, а панель наполняется позже
Гипотезы, которые пришлось отбросить по дороге (каждая проверена в живой игре):
- **не** флаг `wxp_offpanel`: отцеплено=0, прицеплено=71;
- **не** позиция: `GetScreenVector` на слоте отдаёт (11.85, 4.24), не ноль;
- **не** отсутствие `Control`: `Item и Control` есть у всех 71.

Настоящая причина — момент съёмки. `U.refresh()` перестраивает кольцо целиком, но зовут его
только когда фокус потерян (`U.move`, `U.section` — оба через `if table.getn(U.sections) == 0`).
При открытии панели движок ещё не наполнил репозиторий, снимок выходит без сумки, фокус
приземляется на слот снаряжения — и он никуда не девается, поэтому пересборки не случается
НИКОГДА. Доказательство в одну строку: `wxp_ui.refresh()` руками →
`equipment[10] bag[71] quest[11] filters[10] container[1]`.

## Починка: одна отложенная пересборка после смены панели
`U.tick()` (он уже зовётся каждый heartbeat вне мира — ради скрытия подсказки) через 0.4 с после
смены `U.panel` дёргает `U.refresh()` один раз. Фокус при этом не прыгает: `refresh` первым делом
ищет прежний объект через `rawequal` и, найдя, оставляет фокус на нём.
🔴 Метку `U.rebuilt_for` обязательно сбрасывать в `refresh()` при смене имени панели: `U.tick` не
работает в мире, иначе повторное открытие ТОГО ЖЕ инвентаря осталось бы со старым кольцом.

Проверено вживую: при открытии `equipment[10] filters[10] container[1]`, через паузу и **без
единого нажатия** — `equipment[10] bag[71] quest[11] filters[10] container[1]`, фокус остался на
`BackSword`.

## Побочно найдено той же съёмкой
- 🔴 **Запасной путь в `pos_of` мёртв:** `c.m_pModel:GetPosition()` без аргумента — ошибка tolua
  (геттер работает через out-параметр `Vector`), `pcall` её глотает. В этом баге он не участвовал,
  потому что основной путь отработал, но при первом же контроле без `lm_Definition` он подведёт.
- **Правка `wxp_config.ini` руками недолговечна.** Порог бега я выставил в файле, а внутриигровая
  вкладка хранит своё значение в реестре и при первом же «Принять» вернула бы 70. Правильно —
  через объект настройки: `wxp_settings_by_key.RunThreshold:SetValue(85)` + `ApplyChanges()`.
- Подсказка на маркерах карты не поднимается (в отличие от слотов инвентаря) — не разбирался.
- Строка «Геймпад: мёртвая зона правого стика» обрезается по первой букве, соседняя такой же
  длины — нет. Косметика, в очередь.
